...
app.post('/api/addpolygongeofences', async (req, res) => {
  try {
  const { geofenceId, geofenceName, geofenceType, type,seaport, date, remarks, coordinates } = req.body;
...
app.post('/api/addcirclegeofences', async (req, res) => {
    ...
    const { geofenceId, geofenceName,type , geofenceType,seaport, date, remarks, coordinates } = req.body;
...
app.delete('/api/delete-polycircle-geofence/:id', async (req, res) => {
  try {
    ...
    await PolyCircleGeofence.findOneAndDelete({ _id : req.params.id});
    res.status(200).send('Geofence deleted successfully');
  } catch (error) {
    res.status(500).json({ message: 'Error deleting geofence', error });
  }
...
auth: {
  ...
  app.put(`/api/updateVesselLocation/:IMO`, async (req, res) => {
    const { MMSI, TIMESTAMP, LATITUDE, LONGITUDE, COURSE, SPEED, HEADING, NAVSTAT, CALLSIGN, TYPE, A, B, C, D, DRAUGHT, DESTINATION, LOCODE, ETA_AIS, ETA, SRC, ZONE, ECA, DISTANCE_REMAINING, ETA_PREDICTED } = req.body;
  ...
  try {
    ...
    const mappedVoyages = voyages.map(voyage => {
      ...
      return {
        ...
        IMO: voyage.IMO || null,
        port,
        AISETA: trackedVessel.AIS?.ETA || null, 
...
app.post('/api/create-voyages', async (req, res) => {
  try {
    const { vessel, port } = req.body;

    if (!vessel || !port) {
      return res.status(400).json({ error: "Vessel and Port are required" });
    }
