...
const ModifyGeofence = () => {
  ...
  const fetchGeofences = async () => {
    try {
      ...
      const [terrestrialGeofenceResponse ,polygonResponse, polylineResponse, circleResponse,  ] = responses.map(res =>
        res.status === "fulfilled" ? res.value.data : []
      );
  
      const allGeofences = [...terrestrialGeofenceResponse, ...polygonResponse, ...polylineResponse, ...circleResponse ];
                ...
                overflow: "hidden", // Ensures no clipping
                  ...
                  renderOption={(props, item) => (
                    <li {...props} key={item._id}>
                         <span style={{ fontWeight: 'normal' }}> {item.geofenceName}</span> ({item.type || "N/A"} )
                    </li>
                  )}
...
/>
 ...
 {/* Correct way to use FlyToPort */}
              ...
              {ports.map((port, index) => (
                <Marker key={index} position={[port.lat, port.long]} icon={portIcon}>
                  <Popup>
                    <b>{port.name}</b> <br />
                    UNLOCODE: {port.UNLOCODE} <br />
