    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          const errorMessagesByCategory = [];
  
          // Iterate over the response results and categorize the messages
          response.data.results.forEach((result) => {
            if (result.status === 'success') {
              successMessages.push(result.imo); // Collect IMOs for successful deletion
            } else if (result.status === 'error')  {
              
              errorMessagesByCategory.push(result.imo);
           
           
            }
          });
  
          // Prepare the success and error messages
          const allMessages = [];
  
          if (successMessages.length > 0) {
            allMessages.push(`Vessel(s) with IMO ${successMessages.join(", ")} deleted successfully`);
          }

          if (errorMessagesByCategory.length > 0) {
            allMessages.push(`Vessel(s) with IMO ${errorMessagesByCategory.join(", ")} could not be deleted`);
          }
  
        
          // Show Swal with all messages (success and errors)
          Swal.fire({
            title: 'Delete Results',
            text: allMessages.join("\n\n"), // Join success and error messages
            icon: 'info', // or 'success' depending on your preference
            confirmButtonText: 'OK',
          });
  
          // Update the list after deletion by removing the deleted vessels
          setVesselToDelete(vessels.filter(vessel => !vesselToDelete.includes(vessel.imo)));
        }
  
      } catch (error) {
        // Handle any error in case the request fails
